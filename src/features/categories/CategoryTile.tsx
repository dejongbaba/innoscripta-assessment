  ...
  return (
    ...
    >
      {imageUrl && !imageFailed ? <>
        <img src={imageUrl} alt="" loading="lazy" onError={() => setImageFailed(true)} className="absolute inset-0 size-full scale-110 object-cover opacity-0 transition duration-500 ease-out group-hover/tile:scale-100 group-hover/tile:opacity-100 group-focus-visible/tile:scale-100 group-focus-visible/tile:opacity-100" />
        <span aria-hidden="true" className="absolute inset-0 bg-black/40 opacity-0 transition-opacity duration-500 group-hover/tile:opacity-100 group-focus-visible/tile:opacity-100" />
      </> : <div className={`absolute inset-0 bg-gradient-to-br ${tileColors[slug]} opacity-0 transition duration-500 group-hover/tile:opacity-100 group-focus-visible/tile:opacity-100`} />}
